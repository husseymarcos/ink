defmodule Ink.Canvas do
  ...
  @type stroke :: %{
          points: [%{required(String.t()) => number()}],
          created_at: non_neg_integer() | nil
        }
  @type strokes :: [stroke()]

  @doc "Append a new empty stroke to the list."
  @spec start_stroke(strokes()) :: strokes()
  def start_stroke(strokes) do
    strokes ++ [%{points: [], created_at: nil}]
  end
  ...
  @doc "Convert internal stroke structs to list of point lists for the client."
  @spec to_points(strokes()) :: [[%{String.t() => number()}]]
  def to_points(strokes) when is_list(strokes) do
    Enum.map(strokes, & &1.points)
  end
